...
//      MBeanServer mbs = MBeanServerFactory.createMBeanServer();//不能在jconsole中使用  
        ...
        //创建MBean  
        XMBean controller = new X();  
        //将MBean注册到MBeanServer中  
        mbs.registerMBean(controller, new ObjectName("MyappMBean1:name=X"));  
...
//      MBeanServer mbs = MBeanServerFactory.createMBeanServer();//不能在jconsole中使用  
        ...
        XMBean controller = new X();  
        //将MBean注册到MBeanServer中  
        mbs.registerMBean(controller, new ObjectName("SSCMBean:name=SSCBean"));  
          
        //创建适配器，用于能够通过浏览器访问MBean  
        HtmlAdaptorServer adapter = new HtmlAdaptorServer();  
        adapter.setPort(9797);  
        mbs.registerMBean(adapter, new ObjectName(  
                "htmladapter:name=localhost[9797]"));  
        adapter.start();
	}
}
/**
