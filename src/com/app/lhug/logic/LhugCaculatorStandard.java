 ...
 * 创建时间: 2017年11月8日
 */
public class LhugCaculatorStandard {
	Log log = LogFactory.getLog(LhugCaculatorStandard.class);
	static List<TB_A> aList = null;
	...
	public static void calByAccount(String account, int season, boolean check) throws IOException {
		...
		// FileUtils.overideString2File(result, "d://data//lhug//tmp//" +
		// account + "-" + season + ".txt");
		if (check) {
	...
	public static void main(String[] args) throws IOException {
		aList = JSONObject.parseArray(FileUtils.readContentFromFile(DataGeneror.standardTxtFilePath), TB_A.class);
		for (int i = 0; i < 100; i++) {
			long s = System.currentTimeMillis();
			calByAccount("123456", 2, false);
			long e = System.currentTimeMillis();
			System.out.println("共耗时：" + (e - s) / 1000 + "." + (e - s) % 1000);
		}
	}
}
